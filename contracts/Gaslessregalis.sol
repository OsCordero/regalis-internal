...
contract Gaslessregalis is ERC721, VRFConsumerBase, ERC2771Context {
  ...
  constructor(
      ...
      ) ERC721("Regalis", "R") // Initialize Regalis 
      VRFConsumerBase(_VRFCordinator, _LinkToken) // Initialize VRFConsumerBase
        ERC2771Context(address(_minimalForwarder))   // Initialize trusted forwarder
      {
       keyHash = _keyHash;
       fee = _fee;   
    ...
    //overriding context _msgSendr and _msgData to avoid conflicts due to inheritance   
    // Used if you're using GSN contracts instead of open Zepellin ERC2771 context string public override versionRecipient = "2.2.0";
  ...
  function getRandomBox() public returns (bytes32) {
    ...
    requestToSender[requestId] = _msgSender(); //Changed from msg.sender
    return requestId;
  }	
